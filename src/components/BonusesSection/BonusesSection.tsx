...
import { getPlayerBonuses } from "../../services/bonusService";
...
export default function BonusesSection() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getPlayerBonuses(user.id)
      .then((data) => {
        setBonuses(data.unused);
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl lg:rounded-2xl shadow-lg p-3 sm:p-4 lg:p-6 flex flex-col items-center">
      <h2 className="text-lg sm:text-xl lg:text-2xl font-extrabold mb-2 sm:mb-3 lg:mb-4 text-green-700 dark:text-green-200 uppercase tracking-wide">🎁 My Bonuses</h2>
      <div className="flex flex-col w-full gap-6 py-4">
        {userLoading || loading ? (
          <span className="text-gray-400">Loading...</span>
        ) : !Array.isArray(bonuses) || bonuses.length === 0 ? (
          <span className="text-gray-500">No bonuses</span>
        ) : (
          <>
            {[
              ...
              const filtered = bonuses.filter((b) => b.rarity === section.rarity);
